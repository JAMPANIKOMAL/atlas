...
# --- FINAL BUGFIX: Import `save_npz` alongside `load_npz` ---
...
if __name__ == "__main__":
    ...
    train_indices, val_indices = train_test_split(np.arange(X_train_full.shape[0]), test_size=0.1, random_state=RANDOM_STATE, stratify=y_train_full)
    ...
    del X_train_full, y_train_full, X_train_final, y_train_final, X_val_final, y_val_final
    gc.collect()

    training_generator = COISparseDataGenerator(PROCESSED_DATA_DIR / "X_train_final_coi.npz", PROCESSED_DATA_DIR / "y_train_final_coi.npy", BATCH_SIZE)
    validation_generator = COISparseDataGenerator(PROCESSED_DATA_DIR / "X_val_coi.npz", PROCESSED_DATA_DIR / "y_val_coi.npy", BATCH_SIZE)
    ...
    test_generator = COISparseDataGenerator(X_test_path, y_test_path, BATCH_SIZE)
