...
if __name__ == "__main__":
    # --- Step 1: Load Data ---
    print("--- Step 1: Loading Pre-processed ITS Data ---")
    X_train = load_npz(PROCESSED_DATA_DIR / "X_train_its.npz")
    X_test = load_npz(PROCESSED_DATA_DIR / "X_test_its.npz")
    y_train = np.load(PROCESSED_DATA_DIR / "y_train_its.npy")
    y_test = np.load(PROCESSED_DATA_DIR / "y_test_its.npy")
    with open(MODELS_DIR / "its_genus_label_encoder.pkl", 'rb') as f:
        label_encoder = pickle.load(f)
    print(f"  - Loaded {X_train.shape[0]} training samples.")

    # --- Step 2: Define and Compile Model ---
    print("\n--- Step 2: Defining ITS Model Architecture ---")
